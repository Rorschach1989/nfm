...
data_full = SurvivalDataset.metabric('./metabric_IHC4_clinical_train_test.h5')
fold_c_indices = []
fold_ibs = []
...
for _ in tqdm(range(10)):
    ...
    for i in range(5):
        ...
        optimizer = torch.optim.NAdam(lr=1e-3, weight_decay=1e-2, params=list(m.parameters()) + list(nll.parameters()))
        for j in range(1000):
            m.train()
            m_z = m(z)
