...
for i in tqdm(range(1)):
    ...
    for i in range(5):
        ...
        nll = TransNLL(eps_conf=BoxCoxEps(learnable=True), num_jumps=int(train_folds[i].delta.sum()))
        with torch.no_grad():
        ...
        fold_ibs.append(np.asarray(test_ibs)[valid_c_indices])
        fold_inbll.append(np.asarray(test_inbll)[valid_c_indices])
